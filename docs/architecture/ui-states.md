...
| A success is about ONE device | A sticky `verified` (settled from the completed event) is re-proven whenever the connected writable device changes (the route re-hydrates on every `writableDeviceId` change): the connected device is asked again through the live read — the same Lunii, re-detected under its post-write identifier, proves the pack and keeps the terminal (summary refreshed); ANOTHER Lunii plugged in after the send does not hold the pack, so the terminal is withdrawn (`idle`) — a success is never inherited by a different device. A failed re-proof proves nothing either way and keeps the terminal. The V3 pack engine follows the same rule with its own truth: its `sent` terminal (`Pack envoyé sur l'appareil.`) stays only while the connected device's inventory, once read and not being re-read, lists the sent pack — the post-send re-read of the same device lists it, another device's inventory does not. |
...
| Sections | Five `<h2>` sections, in order: `Appareils` (the device support matrix grouped by family — every cohort line with its metadata format label and its four capability lines), `Artefacts locaux` (the three artifact registry lines plus the node-media formats line `Formats acceptés : images PNG, JPEG ; sons MP3, WAV, OGG` VERBATIM), `Association de fichiers` (the four official-channel lines of the file-association registry — frozen Rust-carried labels, status and details, the AppImage line carrying its frozen reason — plus, when the Linux install probe speaks, the current-install notice rendered FIRST in `role="status"`; ABSENT on Windows/macOS and on an indeterminable executable — never an invented claim. Served by the SAME `read_support_profile` pure read, fail-closed WITH the profile sections; a registered channel renders StateChip `success` + its status label, a non-registered one StateChip `neutral` + its status label + its frozen reason — the durable-distribution-limit regime, NEVER `error`/`warning`, never `role="alert"`, and NO toggle: activation belongs to the OS gesture — see [device-support-profile.md#File Association Contract](./device-support-profile.md)), `Sources de contenu` (the read policy rendered as-is: an enabled kind with its Rust-carried frozen entry-level marker `Activée par la distribution officielle` — the DTO's `activationMarker` — non-enabled kinds with their Rust-carried frozen reasons), `Politique de distribution` (the frozen posture copy derived from the PRD distribution policy). |
...
Contract ([device-support-profile.md#File Association Contract](./device-support-profile.md))
— never to this channel.
...
- The zone sits ABOVE the structure (first thing on the editor, whatever the story's length). The `Nœud courant` zone FOLLOWS the scroll (sticky, like the library decision panel): selecting a node deep in a long structure never forces a scroll back up to edit it; taller than the viewport it scrolls internally; stacked (narrow) layouts pin nothing. Layout, one radio choice: `À la suite` (episodes chained in order — the default every story starts with) / `Au choix` (a spoken question, then the episodes on the wheel; at the end of an episode the device returns to the wheel POSITIONED ON THAT EPISODE — its spoken title plays again and the child scrolls on to the next one, never back to the first; the home button returns to the same wheel spot). Under `Au choix` only, a nested checkbox `Passer automatiquement à l'épisode suivant` (hint: `À la fin d'un épisode, la Lunii enchaîne le suivant au lieu de revenir au menu. Le bouton maison ramène toujours au menu.`) — a PER-STORY choice stored through Rust (`set_story_auto_continue`, `story_layouts.auto_continue`, off by default; one child wants the series to roll on, another wants to pick every time): the end of an episode chains straight into the next one's story stage, the last episode returns to the wheel on itself. `Au choix` is disabled while the structure does not lay out as episodes, with the PRECISE reason naming the node to fix: `« Au choix » indisponible : l'épisode « <libellé> » n'a pas d'audio. Ajoute-lui un audio depuis « Structure de l'histoire ».` / `… propose des choix. Retire ses options pour une lecture par épisodes.` / `… l'histoire n'a aucun épisode.` / `… la structure de l'histoire est illisible.` (the generic `La présentation au choix demande un audio sur chaque épisode et aucun choix dans l'histoire.` only when no node can be named). While the story is not editable (recovery pending): `Présentation verrouillée tant que l'histoire n'est pas modifiable (restauration en attente).` A story sent from its retained archive shows only `Cette histoire est envoyée telle quelle depuis son archive d'origine : la présentation ci-dessous ne s'applique pas à l'envoi.`
- Announcements (menu only), folded under a one-line summary (`N annonces : X prêtes, Y à régénérer, Z manquantes`): one row per spoken clip — each with `Enregistrer` (or `Réenregistrer` when a clip exists): the microphone take shows `Dis : « <texte> » — N s` with a pulsing dot and `Arrêter`, then a review line `Prise de N s` with `Écouter la prise` / `Utiliser` / `Refaire` / `Annuler`; an accepted take is stored as the row's clip, chipped `ta voix` (info), never regenerated by a voice (a label change marks it `à régénérer` = to re-record); `Retirer` drops any clip back to `manquante`. Microphone refusals are worded (`L'accès au micro a été refusé…`, `Aucun micro n'a été trouvé…`, `La prise est vide…`) as `role="alert"` on the row, the row stays idle — `Titre de la série`, `Question`, `Épisode N` — with the exact spoken text (« … ») and a state chip: `prête` (success), `à régénérer` (warning: the label or the voice changed since), `manquante` (neutral). `Écouter` per generated clip. One gesture `Générer les annonces` (missing + stale only) and `Régénérer toutes les annonces`; while it runs, a determinate `Génération des annonces…` bar. A failure renders the Rust message plus its next gesture (`role="alert"`).
