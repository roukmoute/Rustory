...
| Order on the device (arrows) | `Monter — <titre>` / `Descendre — <titre>` | Accessible names of the two per-entry arrows that move a device story one step in the device's own play order (the wheel order); an unrecognized pack is named `Histoire non reconnue, identifiant <shortId>` so every name stays unique. Same verbs as the editor's node reordering, reused verbatim | `Déplacer`, `Trier`, drag-and-drop wording, `Réorganiser` as a CTA |
...
| Send refused — the device lacks room (message) | `Envoi impossible: il manque {taille} d'espace libre sur l'appareil.` (`il manque au moins {taille}` when the send stops early, during the transcoding, on what it can already prove) | The pre-write room check: the connected device cannot hold the pack, refused before a single device byte. Sizes are spelled in decimal units with a French comma (`250 Mo`, `1,4 Go`, `moins de 1 Mo`), like the voice-download line | `espace insuffisant` alone, `disque plein`, `erreur d'écriture`, binary units (`Gio`), a raw byte count |
| Send refused — the device lacks room (next gesture) | `Supprime une histoire de l'appareil puis relance l'envoi.` | The real gesture: free room on the device, then send again | `libère de l'espace` (vague), `formate l'appareil`, `réessaie` alone |
...
| Support profile screen (title + nav entry) | `Profil de support` | The `<h1>` of the read-only `/settings` screen AND the label of its permanent library-column navigation entry — one literal, one destination | `Réglages`, `Paramètres`, `Support`, `matrice` |
| Support profile screen — version header | `Version {version}` | Header line carrying the running app version (via `getVersion()`); makes the `consulte le profil de support de ta version` gesture literal. Omitted if the version read fails — never invented | `v{version}` alone, build hashes |
| Support profile screen — section headers | `Appareils` / `Artefacts locaux` / `Association de fichiers` / `Sources de contenu` / `Politique de distribution` | The five frozen `<h2>` of the support-profile screen, in this order | `Matrice`, `Devices`, `Flux`, `Légal` |
| Support profile — capability available (state) | `Disponible` | Calm state chip (`success` tone) of a capability or artifact the distribution supports on this line | `OK`, `activée` (reserved for content sources), `supportée` used loosely |
| Support profile — capability not available (state) | `Non disponible dans cette version` | Calm state chip (`neutral` tone, NEVER `error`/`warning`) of a capability or artifact the current distribution does not offer — the FOURTH vocabulary: a durable distribution limit, never a runtime error | `indisponible` (runtime policy refusal), `échoué` (transport), `bloquée`, `erreur` |
| Support profile — V3 limit reason | `Rétro-ingénierie du format en cours` | Frozen Rust-carried reason on the Lunii V3 import and write capability lines (same rationale as the documented matrix) | `corruption risk` verbatim, `en développement`, `bientôt` |
| Support profile — FLAM write limit reason | `Écriture non prouvée sur matériel réel` | Frozen Rust-carried reason on the FLAM Gen1 write capability line | `non testée`, `dangereuse`, `interdite` |
| Support profile — device cohort labels | `Origine v1` / `Mid-Gen v2` / `V3` / `Gen1` | Frozen user-facing labels of the firmware cohorts, aligned with the documented matrix; family labels stay `Lunii` / `FLAM` | raw wire tags (`origineV1`, `flamGen1`) in UI |
| Support profile — metadata format label | `Format métadonnées v3` / `Format métadonnées v6` / `Format métadonnées v7` | Frozen per-cohort metadata-format line; OMITTED for a family without a documented version byte (FLAM) — never invented | `v3` alone, `metadata_v3`, inventing a FLAM version |
| Support profile — device capability labels | `Lecture bibliothèque appareil` / `Inspection d'histoire` / `Copie dans la bibliothèque locale` / `Transfert vers la Lunii` (Lunii lines) / `Transfert vers l'appareil` (non-Lunii lines) / `Suppression sur l'appareil` / `Envoi d'un pack (.zip)` / `Réordonner les histoires sur l'appareil` | The operation labels of a device matrix line — REUSED VERBATIM from the detection panel's capability lines (same operations, same words); the write label bifurcates family-correctly by construction | inventing a second capability vocabulary for the screen |
| Local artifact type labels (support profile) | `Artefact d'histoire Rustory (.rustory)` / `Dossier structuré` / `Archive structurée` | Frozen labels of the three registry lines of the documented `Supported local artifact types` table | `fichier .rustory`, `zip`, `package` |
| Local artifact capability labels (support profile) | `Import et export` (`.rustory`) / `Création d'une histoire` (dossier structuré) | Frozen capability wording of an AVAILABLE artifact line — aligned word for word with the documented table | `lecture/écriture`, `I/O` |
| Local artifact format label (support profile) | `Format v1` | Frozen format-version line of an artifact registry line that documents one; omitted when the table documents none (archive) | `formatVersion == 1` verbatim in UI |
| Support profile — archive limit reason | `Lecture d'archives non prise en charge` | Frozen Rust-carried reason on the deferred structured-archive line | `zip non supporté`, `règle zéro-dépendance` in user copy |
| File association (section header) | `Association de fichiers` | The fifth read-only `<h2>` of the support-profile screen: whether each official distribution channel registers Rustory as the OS handler for its supported file type — see [device-support-profile.md#File Association Contract](./device-support-profile.md) | `types de fichiers`, `handler`, `enregistrement OS` |
| File association — extension label | `.rustory` | The frozen extension label of the file-association section (the single associated type) | listing several extensions, `*.rustory`, `rustory` bare |
| File association — channel labels | `Paquet Linux (.deb / .rpm)` / `AppImage (Linux)` / `Installeur Windows (.msi / .exe)` / `Application macOS (.dmg)` | Frozen Rust-carried labels of the four official-channel lines of the file-association registry | raw wire tags (`linuxSystemPackage`, `macosAppBundle`…) in UI |
| File association — registered at install (state) | `Enregistrée à l'installation` | Calm status label (`success` chip) of a channel whose package/installer declares the association at install time (Linux system package, Windows installer) | `activée` (reserved for content sources), `Disponible` (reserved for capabilities) |
| File association — registered by the system (state) | `Enregistrée par le système` | Calm status label (`success` chip) of the macOS channel: Launch Services registers the app when it is dropped into Applications | `automatique`, `Disponible` |
| File association — not registered by default (state) | `Non enregistrée d'office` | Calm status label (`neutral` chip) of the AppImage channel — the durable-distribution-limit regime (never `error`/`warning`, never `role="alert"`), yet a NEW literal: it never recycles `Non disponible dans cette version`, `indisponible` or `échoué` | reusing the fourth-vocabulary chip copy, `désactivée`, `impossible` |
| File association — channel details (per-channel, Rust-carried) | `L'association est déclarée par le paquet et active dès l'installation.` / `Une AppImage ne modifie pas ton système : rien n'est enregistré automatiquement.` / `L'installeur déclare l'association. Windows peut te demander de confirmer et respecte ton choix existant.` / `macOS enregistre l'association quand l'application est déposée dans Applications.` | The frozen one-line explanation under each channel line, rendered verbatim | recomposing these copies in the frontend |
| File association — AppImage limit reason (Rust-carried) | `Tu peux ajouter l'association avec un outil d'intégration AppImage ou une entrée d'application manuelle.` | The frozen reason accompanying the AppImage `Non enregistrée d'office` line — a real next gesture, calm tone | `impossible`, transport wording, `échoué` |
| File association — current-install notice (AppImage) | `Ton installation actuelle est une AppImage : l'association n'est pas enregistrée d'office.` | Frozen `role="status"` notice heading the section when the Linux install probe identifies an AppImage run | asserting a state on Windows/macOS |
| File association — current-install notice (system package) | `Ton installation actuelle provient d'un paquet système : l'association est enregistrée.` | Frozen `role="status"` notice when the Linux install probe identifies a system-package install (executable under `/usr/`) | asserting a state on Windows/macOS |
| File association — current-install notice (local build) | `Cette version de Rustory n'a pas été installée par un paquet officiel : elle n'enregistre pas d'association d'office.` | Frozen `role="status"` notice when the Linux install probe identifies an unpackaged local run — it speaks only of what THIS copy provides (no default registration) and never denies an association the user may have added manually; the notice is ABSENT when the executable is indeterminable — never an invented claim | asserting a state on Windows/macOS, guessing on an unknown executable, denying a manual association (`aucune association n'est enregistrée`) |
| Support profile — distribution posture (frozen copy) | `La distribution officielle autorise par défaut les histoires créées dans Rustory, tes contenus personnels et les contenus explicitement libres. Elle n'active jamais de flux orientés vers des contenus protégés non autorisés et n'intègre aucun contournement de protections techniques.` | The 2-sentence posture copy of the `Politique de distribution` section, derived from the PRD official distribution policy; rendered verbatim, never blocking | legalese, consent gates, softening either sentence |
| Support profile — section could not be read (profile) | `Le profil de support n'a pas pu être lu.` | Calm honest copy of the `unavailable` state of the devices + artifacts sections when the pure profile read fails (`role="status"`, no retry) | `erreur`, `échec`, `indisponible`, offering `Réessayer` |
| Support profile — section could not be read (sources) | `Les sources de contenu n'ont pas pu être lues.` | Calm honest copy of the `unavailable` state of the sources section when the pure policy read fails (`role="status"`, no retry) | `erreur`, `échec`, `indisponible`, offering `Réessayer` |
| Settings header — update status line | *(the four Rust-carried headline/notice couples below)* | One `role="status"` line in the `/settings` header, UNDER the `Version {version}` line (which stays verbatim, `getVersion()`-fed, omitted on failure): renders the update-availability verdict when it exists, NOTHING before — see [ui-states.md#Update Availability Contract](./ui-states.md) | a spinner, a waiting state, replacing or rewording the `Version {version}` line |
| Update availability — update available (headline, Rust-carried) | `Nouvelle version disponible : {latest}.` | Frozen composed headline of the `updateAvailable` state (`info` chip — the only chipped state); `{latest}` renders `MAJOR.MINOR.PATCH`, never a `v` prefix | `Mise à jour requise`, urgency wording, `!` |
| Update availability — update available (notice, Rust-carried) | `Ta version actuelle est {current}. Récupère la nouvelle version depuis la page officielle des versions : github.com/roukmoute/Rustory/releases.` | Frozen composed notice of the `updateAvailable` state; the releases-page address is plain TEXT (the product ships no external-browser opener) — the update GESTURE itself is out of scope | a clickable external link, `Télécharger`, `Installer` |
| Update availability — up to date (headline, Rust-carried) | `Aucune version plus récente n'est publiée.` | Frozen headline of the `upToDate` state — logically true INCLUDING when no release is published yet (the no-release world is never presented as an error) | `Tu es à jour !`, celebratory copy, treating no-release as a failure |
| Update availability — up to date (notice, Rust-carried) | `Aucune action n'est nécessaire.` | Frozen notice of the `upToDate` state | `tout va bien`, empty string |
| Update availability — check unavailable (headline, Rust-carried) | `La vérification de version n'a pas pu être faite.` | Frozen headline of the `checkUnavailable` state (transport failure, hostile status, unparsable answer) — calm information, never an alarm | `erreur`, `échec réseau`, `hors ligne`, `réessayer` as a CTA |
| Update availability — check unavailable (notice, Rust-carried) | `Rustory reste pleinement utilisable. La vérification réessaiera au prochain lancement.` | Frozen notice of the `checkUnavailable` state — names the visible bound (one check per launch) and the intact core flow | offering a retry button, apologizing, alarming |
| Update availability — check not run (headline, Rust-carried) | `La vérification de version n'est pas exécutée pour cette copie.` | Frozen headline of the `checkNotRun` state (a development build or a proven unofficial install — the internal motive lives in the diagnostics log, never on the wire: ONE copy couple) | naming the internal motive on the wire, `désactivée`, `impossible` |
| Update availability — check not run (notice, Rust-carried) | `Cette copie de Rustory ne provient pas d'un canal de distribution officiel : aucune vérification réseau n'est effectuée.` | Frozen notice of the `checkNotRun` state — speaks of THIS copy's channel, never a judgment of the copy itself | `version pirate`, `non supportée`, error wording |
| Update availability — library signal gesture (frontend-frozen literal) | `Voir les détails` | The ONE text button of the library's discreet update signal (foot of the left navigation column, rendered ONLY on `updateAvailable`), navigating in-app to `/settings`; `aria-label` `Consulter les détails de la mise à jour` | `Mettre à jour`, `Télécharger`, an external link, a toast |
| Update apply — integrated plan (headline, Rust-carried) | `Cette copie peut installer les mises à jour de Rustory.` | Frozen headline of the integrated plan of the update-apply zone (`ui-states.md#Update Apply Contract`) — names the capability, never urges | urgency wording, `automatique` |
| Update apply — integrated plan (guidance, Rust-carried) | `Le téléchargement vérifie l'authenticité de la mise à jour avant de l'installer.` | Frozen guidance of the integrated plan — names the verification promise of the mechanism | security jargon (`signature Ed25519`), `sécurisé` as a vague claim |
| Update apply — manual plan, package manager (headline, Rust-carried) | `La mise à jour de Rustory passe par ton gestionnaire de paquets.` | Frozen headline of the `package_manager_owned` manual plan — the package manager owns the installation | `impossible`, `non supporté`, error wording |
| Update apply — manual plan, package manager (guidance, Rust-carried) | `Cette copie a été installée comme paquet système : mets-la à jour avec l'outil de ton système, puis relance Rustory.` | Frozen guidance of the `package_manager_owned` manual plan — a real next gesture | naming a specific package manager, `apt`, `dnf` |
| Update apply — manual plan, unofficial install (headline, Rust-carried) | `La mise à jour intégrée n'est pas disponible pour cette copie.` | Frozen headline of the `unofficial_install` manual plan — speaks of THIS copy's channel, never judges the copy | `version pirate`, `non officielle` as a reproach |
| Update apply — manual plan, unofficial install (guidance, Rust-carried) | `Cette copie n'est pas passée par un canal de distribution officiel. La page officielle des versions reste disponible : github.com/roukmoute/Rustory/releases.` | Frozen guidance of the `unofficial_install` manual plan; the releases-page address is plain TEXT (no external-browser opener) | a clickable external link |
| Update apply — manual plan, unproven channel (headline, Rust-carried) | `La mise à jour intégrée n'est pas encore disponible pour cette installation.` | Frozen headline of the `channel_unproven` manual plan (Windows/macOS release, an indeterminable Linux executable) — `pas encore` names the deferred extension honestly | `impossible`, blaming the platform |
| Update apply — manual plan, unproven channel (guidance, Rust-carried) | `Rustory ne peut pas confirmer le canal de cette copie. La page officielle des versions reste disponible : github.com/roukmoute/Rustory/releases.` | Frozen guidance of the `channel_unproven` manual plan | transport wording, `échoué` |
| Update apply — manual plan, unconfigured trust chain (headline, Rust-carried) | `La mise à jour intégrée n'est pas encore activée pour cette copie.` | Frozen headline of the `trust_chain_not_configured` manual plan (a proven AppImage without an embedded public key) | `clé manquante`, security jargon |
| Update apply — manual plan, unconfigured trust chain (guidance, Rust-carried) | `Cette copie ne peut pas vérifier l'authenticité des mises à jour. La page officielle des versions reste disponible : github.com/roukmoute/Rustory/releases.` | Frozen guidance of the `trust_chain_not_configured` manual plan — names the fail-closed reason calmly | `dangereux`, alarming wording |
| Update apply — manual plan, development build (headline, Rust-carried) | `La mise à jour intégrée n'est pas disponible pour un build de développement.` | Frozen headline of the `development_build` manual plan — defensive only (the zone never renders in a dev build: the verdict does not exist there) | `debug`, internal jargon |
| Update apply — manual plan, development build (guidance, Rust-carried) | `Reconstruis Rustory depuis les sources pour obtenir la dernière version.` | Frozen guidance of the `development_build` manual plan | releases-page guidance (a dev build rebuilds, it does not download) |
| Update apply — running, checking (headline, Rust-carried) | `Vérification de la mise à jour en cours…` | Frozen phase headline of the `checking` phase of the gesture in flight | `connexion…`, spinner-only wording |
| Update apply — running, downloading (headline, Rust-carried) | `Téléchargement de la mise à jour en cours…` | Frozen phase headline of the `downloading` phase | `patiente`, apologizing |
| Update apply — running, installing (headline, Rust-carried) | `Installation de la mise à jour en cours…` | Frozen phase headline of the `installing` phase | `presque fini`, celebratory wording |
| Update apply — running (common notice, Rust-carried) | `Tu peux continuer à utiliser Rustory pendant cette opération.` | Frozen common notice of the gesture in flight — names the non-tunnel promise | `ne ferme pas l'application`, blocking wording |
| Update apply — ready to restart (headline, Rust-carried) | `La mise à jour de Rustory est prête.` | Frozen headline of the `readyToRestart` state — sober, no celebration | `Bravo`, `!` |
| Update apply — ready to restart (notice, Rust-carried) | `Redémarre Rustory pour terminer l'installation. Ton travail local reste en place.` | Frozen notice of the `readyToRestart` state — names the needed gesture AND the local-first promise | `redémarrage requis` as an alarm, auto-restart wording |
| Update apply — failed, feed (headline, Rust-carried) | `Le canal de mise à jour n'a pas répondu.` | Frozen headline of the `feed` failure stage (the signed feed could not be consulted) — calm cause | `erreur réseau`, `échec critique` |
| Update apply — failed, feed (notice, Rust-carried) | `Rustory reste sur sa version actuelle. Réessaie plus tard ; la page officielle des versions reste disponible : github.com/roukmoute/Rustory/releases.` | Frozen notice of the `feed` stage — impact (installation intact) + next gesture | alarming, apologizing |
| Update apply — failed, not applicable (headline, Rust-carried) | `La mise à jour n'est pas encore proposée pour cette installation.` | Frozen headline of the `not_applicable` stage (feed consulted, no update applicable to this target/version — the honest divergence window between the information verdict and the feed) | treating the divergence as an error, `incohérence` |
| Update apply — failed, not applicable (notice, Rust-carried) | `La nouvelle version n'est pas encore publiée sur le canal de mise à jour de cette copie. La page officielle des versions reste disponible : github.com/roukmoute/Rustory/releases.` | Frozen notice of the `not_applicable` stage | blaming the release process |
| Update apply — failed, download (headline, Rust-carried) | `Le téléchargement de la mise à jour n'a pas abouti.` | Frozen headline of the `download` failure stage | `erreur`, `échec réseau` |
| Update apply — failed, download (notice, Rust-carried) | `Rustory reste sur sa version actuelle. Vérifie ta connexion, puis réessaie.` | Frozen notice of the `download` stage — impact + next gesture | apologizing, alarming |
| Update apply — failed, verification (headline, Rust-carried) | `L'authenticité de la mise à jour n'a pas pu être confirmée.` | Frozen headline of the `verification` failure stage — NOTHING was applied; never an "install anyway" | `signature invalide`, `fichier corrompu`, alarming security wording |
| Update apply — failed, verification (notice, Rust-carried) | `Rien n'a été installé : Rustory reste sur sa version actuelle. Réessaie plus tard ; la page officielle des versions reste disponible : github.com/roukmoute/Rustory/releases.` | Frozen notice of the `verification` stage — the intact-installation promise leads | minimizing (`petit problème`), blaming |
| Update apply — failed, install (headline, Rust-carried) | `L'installation de la mise à jour n'a pas abouti.` | Frozen headline of the `install` failure stage (applying failed; the current installation stays intact) | `système endommagé`, alarming wording |
| Update apply — failed, install (notice, Rust-carried) | `Ta version actuelle de Rustory reste en place et utilisable. Réessaie, ou passe par la page officielle des versions : github.com/roukmoute/Rustory/releases.` | Frozen notice of the `install` stage | `réinstalle tout`, drastic guidance |
| Update apply — start gesture (frontend-frozen literal) | `Mettre à jour Rustory` | The ONE primary CTA of the integrated plan at rest (`ui-states.md#Update Apply Contract`), visible only because executable; `aria-label` `Télécharger et installer la mise à jour de Rustory`. The `Mettre à jour` interdiction is LIFTED for this single contract — the object is ALWAYS named (`Rustory`), never a bare `Mettre à jour` | a bare `Mettre à jour`, `Télécharger` alone, `Installer` alone |
| Update apply — restart gesture (frontend-frozen literal) | `Redémarrer maintenant` | The restart CTA of the `readyToRestart` state; `aria-label` `Redémarrer Rustory pour terminer la mise à jour` | `Relancer` (reserved for retry vocabulary), auto-restart wording |
| Update apply — defer gesture (frontend-frozen literal) | `Plus tard` | Folds the restart invite into a sober consultable line — the state stays rendered, never re-proposed insistently | `Ignorer`, `Annuler` |
| Update apply — retry gesture (frontend-frozen literal) | `Réessayer la mise à jour` | The retry CTA of a failed gesture (a NEW full cycle) | a bare `Réessayer` (reserved for transport-failure surfaces), `Relancer` (transfer vocabulary) |
| Library left column (accessible name) | `Navigation bibliothèque` | Generalized `aria-label` of the library layout's `<nav>` — the column now carries filters AND navigation entries | `Filtres bibliothèque` (superseded), `menu`, `sidebar` |
...
| OS open intent | `intention d'ouverture` | A file opened through the operating system ("Ouvrir avec Rustory", CLI launch, second launch) delivered to the app; always lands on the library's existing artifact-import review — see [ui-states.md#OS Open Contract](./ui-states.md) | `deep link`, `handler`, `argv`, `événement système` |
| OS open — several files at once (calm limit, Rust-carried by the DTO, rendered verbatim) | `Rustory ouvre un fichier à la fois. Rouvre chaque fichier séparément.` | Frozen copy when one OS gesture hands several files: a calm named limit (`role="status"`), the intent is consumed, NOTHING is partially processed | `erreur`, `échoué`, a toast, processing the first file silently |
| OS open — a library flow is busy (calm limit, frontend-frozen literal, unique) | `Une opération est déjà en cours dans la bibliothèque. Termine-la, puis rouvre le fichier.` | Frozen copy when an OS open intent arrives while an import/creation/transfer is in flight: the intent is declined calmly (`role="status"`) and the LIVING flow is never interrupted | `indisponible` (policy refusal), `échoué` (transport), interrupting or queuing the intent |
| Drop intent | `intention de dépôt` | A file or folder dropped on the Rustory window (native drag-and-drop) delivered to the app; always lands on the library's existing import / folder-creation review — the gesture sibling of `intention d'ouverture`, see [ui-states.md#Drop Intent Contract](./ui-states.md) | `drag-and-drop event`, `DnD`, `glisser-déposer` as a state name |
| Drop — several elements at once (calm limit, Rust-carried by the DTO, rendered verbatim) | `Rustory traite un seul élément déposé à la fois. Dépose chaque élément séparément.` | Frozen copy when one drop gesture hands several elements: a calm named limit (`role="status"`), the intent is consumed, NOTHING is processed (neither the first nor the rest). IDENTITY ≠ COPY: a SISTER literal of the OS-open multi-file copy, never the same words (reopening ≠ dropping) | reusing the OS-open copy, `erreur`, `échoué`, processing the first element silently |
| Drop — a library flow is busy (calm limit, frontend-frozen literal, unique) | `Une opération est déjà en cours dans la bibliothèque. Termine-la, puis dépose à nouveau ton fichier ou ton dossier.` | Frozen copy when a drop intent arrives while an import/creation/transfer/OS-open settlement is in flight: the intent is declined calmly (`role="status"`) and the LIVING flow is never interrupted. SISTER literal of the OS-open busy copy — same family, distinct words | reusing the OS-open busy copy verbatim, `indisponible` (policy refusal), `échoué` (transport), queuing the intent |
| Drop hover overlay (frontend-frozen literal, `aria-hidden`) | `Dépose ton fichier ou ton dossier pour l'analyser` | Frozen copy of the global hover overlay while a drag hovers the window — honest: the drop triggers an ANALYSIS, never a direct import; purely decorative (`aria-hidden`, no focus, no pointer events) | `Déposer pour importer` (a drop never imports directly), `lâcher`, `drop here` |
...
| Activation marker (entry level, Rust-carried) | `Activée par la distribution officielle` | The frozen entry-level marker of an enabled content-source line — CARRIED BY THE POLICY DTO (`activationMarker`, present IFF `enabled`, the exact complement of `reason`) and rendered VERBATIM by every consuming surface (creation dialog AND support-profile screen); the remaining TS literal is a guard VALIDATION value only. Same copy family as the surface mention, DISTINCT frozen literal (no final period) | reusing the surface mention verbatim at entry level, re-typing the marker in a frontend surface |
...
| External sources unavailable — fail-closed (reason) | `Sources externes indisponibles pour l'instant.` | Frozen fail-closed reason when the policy read failed or is absent: every external-source entry renders disabled; the only content-source copy rendered WITHOUT a successful policy read (the surface-level activation mention stays a frontend-frozen literal, while the entry-level marker is Rust-carried by the policy DTO — both only accompany a successfully read `enabled` line) | leaving entries active on a failed read |
...
| Presentation layout — menu | `Au choix` | A spoken question then the episodes on the wheel, like an official pack; the end of an episode returns to the wheel on that episode (hint: `L'enfant choisit l'épisode sur la molette ; à la fin, la Lunii revient au menu sur cet épisode.`) | `menu`, `sélection`, `carousel` |
| Menu layout — chain the next episode (per-story option) | `Passer automatiquement à l'épisode suivant` | Checkbox under `Au choix`: at the end of an episode the device plays the next one instead of returning to the wheel (hint: `À la fin d'un épisode, la Lunii enchaîne le suivant au lieu de revenir au menu. Le bouton maison ramène toujours au menu.`) | `autoplay`, `lecture continue`, `enchaînement` as a label |
...
| OS open — several files handed in one gesture (calm limit, Rust-carried) | `Rustory ouvre un fichier à la fois. Rouvre chaque fichier séparément.` |
| OS open — a library flow is already in flight (calm limit, frontend-frozen) | `Une opération est déjà en cours dans la bibliothèque. Termine-la, puis rouvre le fichier.` |
| Drop — several elements handed in one gesture (calm limit, Rust-carried) | `Rustory traite un seul élément déposé à la fois. Dépose chaque élément séparément.` |
| Drop — a library flow is already in flight (calm limit, frontend-frozen) | `Une opération est déjà en cours dans la bibliothèque. Termine-la, puis dépose à nouveau ton fichier ou ton dossier.` |
| Drop hover overlay (decorative, `aria-hidden`) | `Dépose ton fichier ou ton dossier pour l'analyser` |
...
| Content-source activation marker (entry level — Rust-carried by the policy DTO, rendered verbatim by the creation dialog and the support-profile screen) | `Activée par la distribution officielle` |
...
| Support profile — capability or artifact available (calm chip, `success` tone) | `Disponible` |
| Support profile — capability or artifact not available in this distribution (calm chip, `neutral` tone — the fourth vocabulary, never an error) | `Non disponible dans cette version` |
| Support profile — Lunii V3 import/write limit (Rust-carried reason) | `Rétro-ingénierie du format en cours` |
| Support profile — FLAM Gen1 write limit (Rust-carried reason) | `Écriture non prouvée sur matériel réel` |
| Support profile — structured-archive deferral (Rust-carried reason) | `Lecture d'archives non prise en charge` |
| Support profile — devices/artifacts sections could not be read (calm `role="status"`, no retry) | `Le profil de support n'a pas pu être lu.` |
| Support profile — sources section could not be read (calm `role="status"`, no retry) | `Les sources de contenu n'ont pas pu être lues.` |
| File association — section header (fifth `<h2>` of the support profile) | `Association de fichiers` |
| File association — channel registered at install (calm `success` chip) | `Enregistrée à l'installation` |
| File association — channel registered by the system (calm `success` chip, macOS) | `Enregistrée par le système` |
| File association — channel not registered by default (calm `neutral` chip, AppImage; a NEW literal of the durable-limit regime) | `Non enregistrée d'office` |
| File association — AppImage limit reason (Rust-carried, next gesture) | `Tu peux ajouter l'association avec un outil d'intégration AppImage ou une entrée d'application manuelle.` |
| File association — current install is an AppImage (Linux probe, `role="status"`) | `Ton installation actuelle est une AppImage : l'association n'est pas enregistrée d'office.` |
| File association — current install is a system package (Linux probe, `role="status"`) | `Ton installation actuelle provient d'un paquet système : l'association est enregistrée.` |
| File association — current install is an unpackaged local build (Linux probe, `role="status"`) | `Cette version de Rustory n'a pas été installée par un paquet officiel : elle n'enregistre pas d'association d'office.` |
...
- **`Non disponible dans cette version` is the FOURTH vocabulary** (support-profile screen): a durable DISTRIBUTION limit, calm by contract (`neutral` chip, `role="status"`, its own frozen reason). It never replaces and is never replaced by: `indisponible` (runtime policy refusal — `Source indisponible: …`, `Envoi indisponible: …`), `échoué` (transport failure), the typed content verdicts, or the detection panel's `raison de non-support` (a CONNECTED device refused at runtime). Four regimes, four vocabularies — each with its own words, tone and tests; a matrix line never borrows a runtime refusal copy.
- **The two OS-open copies are CALM LIMITS, not a fifth error regime** (`OS Open Contract`): the multi-file copy (`Rustory ouvre un fichier à la fois. …` — Rust-carried by the DTO) and the busy copy (`Une opération est déjà en cours…` — a frontend-frozen literal, typed exactly once) render `role="status"`, never `échoué` (transport), never `indisponible` (policy), never a content verdict. NO collision by construction: an invalid/unsupported file opened through the OS reuses the import findings (`finding_message`) VERBATIM; an unreadable file reuses the transport regime (`Import impossible` + `Réessayer`); no existing import copy is re-typed anywhere in the channel.
- **The drop-channel copies follow the exact same regime discipline** (`Drop Intent Contract`): the multi-element copy (`Rustory traite un seul élément déposé à la fois. …` — Rust-carried by the DTO), the busy copy (`Une opération est déjà en cours dans la bibliothèque. Termine-la, puis dépose à nouveau ton fichier ou ton dossier.` — a frontend-frozen literal, typed exactly once) and the hover-overlay copy (`Dépose ton fichier ou ton dossier pour l'analyser` — frontend-frozen, `aria-hidden`) are calm limits/affordances — never `échoué`, never `indisponible`, never a content verdict. **IDENTITY ≠ COPY**: the OS-open copies are NOT reused (reopening a file ≠ dropping one — sister literals, each channel its own words). Content verdicts stay the existing findings VERBATIM (`finding_message`, the folder copies); an unreadable element stays the transport regime (`Réessayer` replays the pending intent).
- **The file-association copies follow the durable-distribution-limit REGIME with NEW literals** (`File Association Contract`): calm `success`/`neutral` chips, `role="status"` for the current-install notice, never `error`/`warning`, never `role="alert"` — a channel that does not register by default is durable distribution information, not a runtime failure. IDENTITY ≠ COPY: `Non enregistrée d'office`, the channel details, the AppImage reason and the three install notices are their OWN frozen literals — they never recycle `Non disponible dans cette version` (capability/artifact lines), `indisponible` (runtime policy refusal), `échoué` (transport), nor any os-open/drop copy.
- **The update-availability copies are a CALM-INFORMATION regime with NEW literals** (`Update Availability Contract`): four Rust-carried headline/notice couples (one per sealed wire state), `role="status"` on both surfaces, `info` chip on the available state ONLY, never `error`/`warning`, never `role="alert"`, never a toast or a modal. IDENTITY ≠ COPY: none of these literals recycles the runtime-error vocabulary (`échoué`, `erreur`), the durable-distribution-limit vocabulary (`Non disponible dans cette version`, `Non enregistrée d'office`), `indisponible` (policy refusal), the current-install notices, nor any os-open/drop copy. `checkUnavailable` says the CHECK could not be done — it never says the app or a source failed; `checkNotRun` speaks of this copy's channel — it never judges the copy.
- **The update-apply copies are the CALM-GESTURE regime with NEW literals — and a SCOPED lift of the CTA interdictions** (`Update Apply Contract`): the `Mettre à jour` / `Télécharger` / `Installer` interdictions carried by the availability surfaces are lifted FOR THIS SINGLE contract's gesture zone — and remain in force everywhere else, including on both availability surfaces (whose own forbidden columns above stay verbatim). Non-collision rules, NAMED: (a) no literal of the availability contract is recycled — in particular `Récupère la nouvelle version depuis la page officielle des versions : …` stays the EXCLUSIVE property of the `updateAvailable` notice; the gesture copies say `La page officielle des versions reste disponible : …`; (b) the transfer summary's `mise à jour` (replacing an on-device pack) never collides: the gesture copies name `Rustory` or `la mise à jour de Rustory` explicitly, never a bare `mise à jour` where the transfer sense could resonate; (c) the failure notices state the intact installation (`Rustory reste sur sa version actuelle.` / `Ta version actuelle de Rustory reste en place et utilisable.`) — never the runtime-error vocabulary, never `indisponible`, never a durable-limit literal. The manual-plan copies are durable-distribution-limit INFORMATION (calm `role="status"`, no button); the failure copies are stage-scoped cause/impact/next-gesture couples; both are their OWN frozen literals.
