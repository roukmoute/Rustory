...
import type {
  ...
  AttachRecordedAnnouncementInput,
...
/**
 * Attach a microphone recording (mono WAV, base64) as one announcement. It
 * replaces the previous clip of that target and is never regenerated by a
 * voice afterwards.
 */
export function attachRecordedAnnouncement(
  input: AttachRecordedAnnouncementInput,
): Promise<StoryPresentationDto> {
  return call("attach_recorded_announcement", { input }, isStoryPresentationDto);
}

/** Remove one announcement's clip (recorded or generated): back to missing. */
export function removeStoryAnnouncement(
  input: RemoveAnnouncementInput,
): Promise<StoryPresentationDto> {
  return call("remove_story_announcement", { input }, isStoryPresentationDto);
}
