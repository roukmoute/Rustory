-- Let the local-import provenance carry the structured-folder creation flow:
-- the explicitly listed format set gains 'structured-folder'. SQLite cannot
-- alter a CHECK in place, so the table is REBUILT (same recipe as 0010): same
...
CREATE TABLE story_local_imports_new (
  ...
  -- writes 'rustory' / 'structured-folder', so the schema refuses anything
  -- else (no implicit format).
  CHECK (source_format IN ('rustory', 'structured-folder')),
