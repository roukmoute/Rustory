-- Let the local-import provenance carry the RSS external-source creation
-- flow: the explicitly listed format set gains 'rss'. SQLite cannot alter a
-- CHECK in place, so the table is REBUILT (same recipe as 0010/0011): same
-- columns, same FK, every other CHECK VERBATIM — only the source_format set
-- widens. The child-only recipe (no table references story_local_imports) is
...
CREATE TABLE story_local_imports_new (
  ...
  -- writes 'rustory' / 'structured-folder' / 'rss', so the schema refuses
  -- anything else (no implicit format).
  CHECK (source_format IN ('rustory', 'structured-folder', 'rss')),
