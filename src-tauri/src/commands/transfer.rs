//! Transfer command layer (story preparation).
//!
//! The thin Tauri boundary for the preparation flow. `start_prepare_story`
//! validates the input, generates a `job_id`, kicks the work onto a background
//! task and returns an acceptance IMMEDIATELY; progress is reported through the
//! typed `job:*` events. `read_preparation_state` is the authoritative re-read.
//!
//! The runtime lives HERE: the [`TauriPreparationEmitter`] is the only place
//! that constructs event payloads and calls `AppHandle::emit`; the application
//! service only ever sees the
//! [`PreparationEventEmitter`](crate::application::transfer::PreparationEventEmitter)
//! trait. The DB mutex is never held across the scan or the assembly (the
//! service scopes it), and no `mount_path` crosses the IPC boundary.

use std::time::Instant;

use tauri::{AppHandle, Emitter, Manager, State};

use crate::application::transfer::{PreparationEventEmitter, PreparationOutcome};
...
use crate::ipc::dto::{
    PreparationStateDto, ReadPreparationStateInputDto, StartPreparationAcceptedDto,
    StartPrepareStoryInputDto,
};
use crate::ipc::events::{
    JobCompletedEvent, JobFailedEvent, JobProgressEvent, EVENT_JOB_COMPLETED, EVENT_JOB_FAILED,
    EVENT_JOB_PROGRESS, JOB_TYPE_PREPARE_STORY, PREPARATION_FAILED_CODE,
};
use crate::AppState;

/// Tauri implementation of the preparation event sink. Closes over the
/// correlation identifiers; the application passes only the phase / sequence /
/// message. `emit` is best-effort (a dropped event surfaces via the
/// authoritative re-read).
struct TauriPreparationEmitter {
    app: AppHandle,
    job_id: String,
    story_id: String,
}

impl PreparationEventEmitter for TauriPreparationEmitter {
...
pub async fn start_prepare_story(
    ...
    tauri::async_runtime::spawn(async move {
        ...
        let emitter = TauriPreparationEmitter {
            app: task_app.clone(),
